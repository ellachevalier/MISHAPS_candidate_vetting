...
def find_period(transit_data, data, avg_first_and_last, t, t_err, all_nights, period_guess, rldc, ecc=0.0, w=0.0, exclude=None, cutoff=100, final_cutoff=10):
    delta_t=t[-1]-t[0]
...
#     plt.savefig(os.path.join('period_fit_results','n_vs_x2_before_cutoff.pdf'))
    ...
    for idx in range(len(df)):
        if idx == 0:
            ...
            if df['x2'].iloc[idx]<=df['x2'].iloc[idx-1] and df['x2'].iloc[idx]<=df['x2'].iloc[idx+1] and df['Period'].iloc[idx]<10:
                min_periods.append(df['Period'].iloc[idx])
                min_x2s.append(df['x2'].iloc[idx])
            if abs(df['Period'].iloc[idx]-df['Period'].iloc[idx+1])>0.1 and abs(df['Period'].iloc[idx]-df['Period'].iloc[idx-1])>0.1 and df['Period'].iloc[idx]<10:
                min_periods.append(df['Period'].iloc[idx])
                min_x2s.append(df['x2'].iloc[idx])
    ...
    #print(min_x2s_final)
    df_peaks=df_peaks.sort_values(by='x2')
    #print(df_peaks)
    ...
    #periods_old=list(df_peaks['Period'])
    #print(final_periods)
    df_peaks=df_peaks[df_peaks['x2']<(df_peaks['x2'].min()+4*chi2)]
    df_peaks=df_peaks.sort_values(by='x2')
...
#     #plt.show()
    ...
    periods_new.extend(df_peaks['Period'].tolist())
    periods_new=list(set(periods_new))
    ...
    #periods_new=[2.251933600015337,1.5898767500584126,1.2286325797618285,1.1259603006137333,0.8448145819666568]
    show_plot=False
    #periods_new.append(1.351946)
    for period in periods_new:
    #for period in [2.9409785015697048]:
        #period=delta_t/n
...
#                 min_x2s_final.append(final_df['x2_transit'].iloc[idx])
    ...
    #plt.show()
    #print(final_df_peaks)
    final_periods=list(set(final_periods))
    print('final_periods:')
    print(final_periods)
    if len(final_periods)>5:
        final_periods=final_periods[:5]
